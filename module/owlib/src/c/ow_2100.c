...
static struct aggregate A2100 = { 2, ag_letters, ag_aggregate, };
static struct filetype DS2100[] = {
	F_STANDARD,
	...
	{"PIO", PROPERTY_LENGTH_BITFIELD, &A2100, ft_bitfield, fc_link, FS_r_pio, FS_w_pio, VISIBLE, NO_FILETYPE_DATA, },
	{"sensed", PROPERTY_LENGTH_BITFIELD, &A2100, ft_bitfield, fc_link, FS_sense, NO_WRITE_FUNCTION, VISIBLE, NO_FILETYPE_DATA, },
//	{"latch", PROPERTY_LENGTH_BITFIELD, &A2100, ft_bitfield, fc_link, FS_r_latch, FS_w_pio, VISIBLE, NO_FILETYPE_DATA, },
};

DeviceEntryExtended(85, DS2100, DEV_resume | DEV_ovdr, NO_GENERIC_READ, NO_GENERIC_WRITE);

#define _1W_PIO_ACCESS_READ 0xF5
#define _1W_PIO_ACCESS_WRITE 0x5A

#define _1W_2100_LATCH_MASK 0x0A
#define _1W_2100_CONFIRMATION 0xAA
...
/* 2100 switch activity latch is actual set state of PIOs, what we want in this case */
/* bites 1 and 3 */
...
static GOOD_OR_BAD OW_write(BYTE data, const struct parsedname *pn)
{
	...
	BYTE chk[1];
	BYTE confirm[] = { _1W_2100_CONFIRMATION, } ;
	struct transaction_log t[] = {
		TRXN_START,
		TRXN_WRITE3(cmd),
		TRXN_READ1(chk),
